...
public class CustomerRepository : ICustomerRepository
{
    ...
    //(There are only 91 customers in the db, very few, so we can store the whole table in memory.)
    // => scalability and performance. In real life we would use something like Redis.
    ...
    public CustomerRepository(NorthwindContext injectedContext)
    {
        ...
        {
            //ToDictionary converts a collection into a dictionary, c is the value and CustomerId as the key)
    ...
    public Task<Customer?> RetrieveAsync(string id)
    {
        ...
        return Task.FromResult(c); //this wraps the result in a task, to make something syncronic fit in into the async structure.
    }
